...
Регистрация записывает и то, на что опиралась обученная версия, — поэтому она
может объяснить себя, не загружаясь:
...
Те же числа рисует страница модели в админке. Откуда берутся имена, зависит от
пайплайна: у векторайзера спрашивают его собственный `get_feature_names_out()`,
поэтому текстовая модель называет слова, а не индексы колонок; во всех остальных
случаях имена приходят из объявленных полей.

Бэкенд подключается одним методом:
...
Вернуть `None` — поведение по умолчанию — правильный ответ для бэкенда, чьи веса
не соответствуют ничему, что человек назвал бы признаком; это большинство
нейросетей. Неверное объяснение хуже, чем никакого, поэтому выдумывать список
никого не просят.
